...
const BloodTestEducationalInfoSchema = z.object({
  ...
  whatItIs: z.object({
    ...
    bodySystem: z.string().optional().describe(
      "Optional: The primary body system or function related to this test (e.g., 'Blood Cell Production', 'Kidney Function')."
    ),
  }).describe("Explains the test's identity, purpose, and related body system."),
  understandingResults: z.object({
    normalRangeExplanation: z.string().describe(
    ...
    unitsExplained: z.string().optional().describe(
  ...
  abnormalResults: z.object({
    low: z.object({
        title: z.string().default("What Lower Than Normal Might Mean").describe("Section title for low results."),
        potentialCauses: z.string().describe(
            "Common potential reasons why a result might be lower than normal. Uses cautious, non-diagnostic language."
        ),
        potentialImplications: z.string().describe(
            "Simple explanation of what low levels might mean for health or daily life, avoiding alarmist language."
        ),
    }).optional().describe("Information about results potentially below the normal range."),
    high: z.object({
        title: z.string().default("What Higher Than Normal Might Mean").describe("Section title for high results."),
        potentialCauses: z.string().describe(
    ...
    }).optional().describe("Information about results potentially above the normal range."),
    generalDisclaimer: z.string().describe(
  ...
  symptomsAndActions: z.object({
    associatedSymptoms: z.string().optional().describe(
    ...
    whenToSeekHelp: z.string().describe(
  ...
  }).describe("Covers potential symptoms and provides clear guidance on seeking medical advice."),
  lifestyleTips: z.object({
    title: z.string().default("Lifestyle and Wellness Tips").describe("Section title for lifestyle tips."),
    generalTips: z.string().describe(
    ...
    preventionFocus: z.string().optional().describe(
  ...
  additionalResources: z.object({
    ...
    finalDisclaimer: z.string().describe(
  ...
  }).describe("Provides supplementary information like definitions, FAQs, and the essential final disclaimer.")
});
...
export async function getTestActionDetails(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  let report: (IReport & import("mongoose").Document<Types.ObjectId, any, IReport>) | null = null;
  ...
  try {
    ...
    if (testResultRef.educationalInfo) {
      ...
      const { object: generatedInfoObject } = await generateObject({ 
         ...
         Patient details for context (use age/gender if relevant): ${JSON.stringify(patientDetails)}
      ...
      console.log(`Generated structured info for test ID '${testId}':`, generatedInfoObject);
      ...
      console.log(`Saved generated info for test ID '${testId}' to report '${reportId}'.`);
