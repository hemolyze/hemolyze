...
export function FileUploadCard() {
    ...
    const createReportRecord = async (uploadedFilesData: { fileName: string; filePath: string; fileType: string; fileSize: number }[]) => {
        setIsSaving(true);
        setError(null);
        try {
            const response = await axios.post('/api/report-upload/create-record', {
                uploadedFiles: uploadedFilesData,
            });

            toast.success('Report record created successfully!');
            setFiles([]); // Clear files on successful save
            // Optional: Redirect
    ...
    const handleUpload = useCallback(async () => {
        ...
        const uploadPromises = filesToUpload.map(async (file): Promise<{ success: boolean; fileName: string; filePath?: string; fileType?: string; fileSize?: number; error?: string }> => {
            ...
            try {
                ...
                // Return necessary data for DB record creation
                return { success: true, fileName: file.name, filePath: filePath, fileType: file.type, fileSize: file.size };
        ...
        const successfulUploads = results.filter((r): r is { success: true; fileName: string; filePath: string; fileType: string; fileSize: number } => r.success && !!r.filePath);
        const failedUploads = results.filter(r => !r.success);

        if (failedUploads.length > 0) {
            const errorMsg = `${failedUploads.length} file(s) failed to upload. Check individual file status.`;
            setError(errorMsg);
            toast.error(errorMsg);
        }
        // If all uploads succeeded (no failures)
        else if (successfulUploads.length > 0) {
            setError(null);
            console.log("All files uploaded successfully:", successfulUploads);
            // Prepare data and call createReportRecord
            const filesForRecord = successfulUploads.map(f => ({
                fileName: f.fileName,
                filePath: f.filePath,
                fileType: f.fileType,
                fileSize: f.fileSize,
            }));
            await createReportRecord(filesForRecord); // Call the function here
        }
        else if (filesToUpload.length > 0) {
            setError("Upload process completed, but no files were successfully uploaded.");
        }
                ...
                {files.length > 0 && (
                    <div className="space-y-4">
                                    ...
                                    <li key={`${file.name}-${file.lastModified}`} className="flex items-center justify-between space-x-3">
                                        ...
                                        <div className="flex items-center space-x-2 flex-shrink-0">
                                            ...
                                            {file.uploadProgress.status === 'uploading' && file.uploadProgress.source && (
                                                <Button variant="ghost" size="icon" className="w-6 h-6" onClick={() => file.uploadProgress.source?.abort()} aria-label={`Cancel upload for ${file.name}`} disabled={isSaving}>
                        ...
                        <div className="flex justify-end space-x-3">
                            <Button variant="outline" onClick={() => setFiles([])} disabled={isUploading || isSaving}>Clear All</Button>
                            <Button
                                onClick={handleUpload}
                                disabled={files.filter(f => f.uploadProgress.status === 'pending' || f.uploadProgress.status === 'error').length === 0 || isUploading || isSaving}
                            >
                                {isUploading
                                    ? 'Uploading...'
                                    : isSaving
                                        ? 'Saving Report...'
                                        : `Upload ${files.filter(f => f.uploadProgress.status === 'pending' || f.uploadProgress.status === 'error').length} File(s)`}
                            </Button>
                        </div>
                    </div>
                )}
